...
def get_db_params():
    pg_params, vertica_params = (
        {
            f"{param.lower()}": Variable.get(f"{db_type}_{param}", "") 
            for param in ("DBNAME", "HOST", "PORT", "USER", "PASSWORD")
        } 
        for db_type in ("PG", "VERTICA")
    )
    ...
    return pg_params, vertica_params

def get_params():
    pg_params, vertica_params = get_db_params()
